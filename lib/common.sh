...
# --- Terminal candies: colors + your hog face ---
# Respect NO_COLOR / non-TTY. Logs stay plain (strip codes).
# Hog face is the product mascot for task rows (same as janitor hogs / catalog).

HOG_EMOJI="${HOG_EMOJI:-"{^oo^}"}"
...
# Status still shown in [ok]/[dry run]/…; face is always the hog.
K_FACE="$HOG_EMOJI"
K_SWEEP="$HOG_EMOJI"
K_OK="$HOG_EMOJI"
K_DRY="$HOG_EMOJI"
...
# Human-readable size of a path (n/a if missing)
path_size() {
 local path="$1" kb
 if [[ ! -e "$path" ]]; then
 echo "n/a"
...
log_task() {
 ...
 dry) mark="dry run"; color="$C_YELLOW" ;;
 skip) mark="skip"; color="$C_DIM" ;;
 refuse) mark="refuse"; color="$C_RED" ;;
 done) mark="done"; color="$C_GREEN" ;;
 *) mark="$kind"; color="$C_CYAN" ;;
 esac
 path_show="$path"
 if [[ "$path" != "-" && -n "$path" ]]; then
...
ram_report() {
 ...
 out+=("Available approx: $(fmt_kb "$AVAIL_KB")")
